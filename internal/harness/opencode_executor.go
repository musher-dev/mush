...
// OpenCodeExecutor runs jobs via OpenCode CLI.
...
func init() {
	Register(Info{
		Name:      "opencode",
		Available: AvailableFunc("opencode"),
		New:       func() Executor { return &OpenCodeExecutor{} },
		MCPSpec: &MCPSpec{
			Def:         mustGetProvider("opencode").MCP,
...
func (e *OpenCodeExecutor) Setup(ctx context.Context, opts *SetupOptions) error {
	e.opts = *opts
...
func (e *OpenCodeExecutor) Execute(ctx context.Context, job *client.Job) (*ExecResult, error) {
	if e.opts.BundleDir != "" {
	...
	if runErr != nil {
		if ctxErr := ctx.Err(); ctxErr != nil {
			if errors.Is(ctxErr, context.DeadlineExceeded) {
				return nil, &ExecError{Reason: "timeout", Message: "opencode execution timed out", Retry: true}
			}
...
func (e *OpenCodeExecutor) Reset(_ context.Context) error {
	return nil
}

// Resize implements Resizable for interactive bundle sessions.
func (e *OpenCodeExecutor) Resize(rows, cols int) {
	e.mu.Lock()
	ptmx := e.ptmx
	e.mu.Unlock()
...
func (e *OpenCodeExecutor) WriteInput(p []byte) (int, error) {
	e.mu.Lock()
	ptmx := e.ptmx
	e.mu.Unlock()
...
func (e *OpenCodeExecutor) Interrupt() error {
	e.mu.Lock()
	ptmx := e.ptmx
	e.mu.Unlock()
...
func (e *OpenCodeExecutor) NeedsRefresh(cfg *client.RunnerConfigResponse) bool {
...
func (e *OpenCodeExecutor) ApplyRefresh(_ context.Context, cfg *client.RunnerConfigResponse) error {
	return e.applyRunnerConfig(cfg)
}

func (e *OpenCodeExecutor) applyRunnerConfig(cfg *client.RunnerConfigResponse) error {
	now := time.Now()

	info, ok := Lookup("opencode")
...
func (e *OpenCodeExecutor) startInteractive(ctx context.Context, opts *SetupOptions) error {
	args := []string{}
	if opts.BundleDir != "" {
...
func (e *OpenCodeExecutor) Teardown() {
	e.mu.Lock()
	cmd := e.cmd
	ptmx := e.ptmx
	pgid := e.pgid
...
var (
	_ Executor         = (*OpenCodeExecutor)(nil)
	_ InputReceiver    = (*OpenCodeExecutor)(nil)
	_ Resizable        = (*OpenCodeExecutor)(nil)
	_ InterruptHandler = (*OpenCodeExecutor)(nil)
	_ Refreshable      = (*OpenCodeExecutor)(nil)
)
